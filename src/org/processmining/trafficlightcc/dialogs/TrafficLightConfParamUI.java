...
import org.processmining.partialorder.dialogs.PartialNetAlgorithmStep;
...
/**
 * Copied from Xixi's {@link PNReplayerUI} (transitive copy from Arya's alignment UI).
 * Augmented by an additional page to configure automatic cost adaptation to 
 * practically resolve a bit of the non-determinism.
 * 
 * @author brockhoff
 *
 */
public class TrafficLightConfParamUI {
	...
	public static final int COSTADAPTATION = 3;


	// steps
	private int currentStep;
	private int nofSteps = 3; // algorithm and parameter and cost adaptation
	...
	public Object[] getConfiguration(UIPluginContext context, PetrinetGraph net, XLog log) {
		...
		checkExistenceOfInitialMarking(context, net);

		// check existence of final marking
		checkExistenceOfFinalMarking(context, net);

		// check connection in order to determine whether mapping step is needed
		TransEvClassMapping mapping = checkExistenceOfMapping(context, net, log);

		replaySteps = new JComponent[nofSteps];
		replaySteps[0] = new PartialNetAlgorithmStep(context, net, log, mapping);
		// 1 not yet known
		replaySteps[2] = new CostAdaptStep();
	...
	private TransEvClassMapping checkExistenceOfMapping(UIPluginContext context, PetrinetGraph net, XLog log) {
	...
	private void checkExistenceOfFinalMarking(UIPluginContext context, PetrinetGraph net) {
		try {
	...
	private void checkExistenceOfInitialMarking(UIPluginContext context, PetrinetGraph net) {
		try {
	...
	private Object[] showConfiguration(UIPluginContext context, XLog log, PetrinetGraph net, TransEvClassMapping mapping) {
		...
		while (true) {
			...
			result = context.showWizard("Traffic Light Petri Net Replay", currentStep == 0, currentStep == nofSteps - 1,
			...
			switch (result) {
				...
				case PREV :
					...
					break;
				case FINISHED :
					return new Object[] { mapping, ((PartialNetAlgorithmStep) replaySteps[0]).getAlgorithm(),
							paramProvider.constructReplayParameter(replaySteps[1]), 
							((CostAdaptStep) replaySteps[2]).getCostAdaptation() };
				default :
	...
	private int go(int direction, UIPluginContext context, PetrinetGraph net, XLog log, TransEvClassMapping mapping) {
		...
		if (currentStep == 1) {
			this.paramProvider = ((PartialNetAlgorithmStep) replaySteps[0]).getAlgorithm().constructParamProvider(
